...
--8. Display the employee name, job and start date of employees hired between February 20, 1981 and May 1, 1981. Order the query in ascending order of start date.
  ...
  FROM EMP WHERE HIREDATE BETWEEN '1981-02-20' AND '1981-05-01'ORDER BY HIREDATE;
...
--9. Display the employee name and department number of all employees in departments 10 and 30 in alphabetical order by name.
  ...
  SELECT	ENAME,DEPTNO
  FROM EMP WHERE DEPTNO= 10 OR DEPTNO = 30  ORDER BY ENAME;
...
--13. Display the employees who were hired before their managers.
   ...
   FROM EMP e,EMP m WHERE e.EMPNO = m.MGR AND e.HIREDATE < m.HIREDATE;

--14. Determine the number of managers without listing them.
   
   SELECT COUNT(JOB) AS 'NO_OF_MANAGER'FROM EMP WHERE JOB = 'MANAGER';
...
SELECT	COUNT(ENAMR) FROM EMP JOIN DEPT ON LOC='CHICAGO';
...
SELECT	ENAME,LOC FROM EMP E JION DEPT D ON E.DEPTNO=D.DEPTNO
ORDER BY LOC; 
...
SELECT	TOP 1 ENAME,avg(sal) as average_salary from emp group by ename order by average_salary;


--31.	Determine number of departments in the company.

select count(*) as DEPTNO from EMP
...
from emp e,dept group by e.DEPTNO,DNAME,LOC order by DEPTNO;

--33.Display the employee name, job description and the salary multiplied by 12.

SELECT	ENAME,JOB,SAL*12 AS 'ANUAL SALARY' FROM EMP;
...
select sal,ename, (case when sal = any (select sal from emp where 1250>sal) then 'lessthan1250'
when sal = any (select sal from emp where 1600<sal) then 'morethan1600' end) as 'status-result'
from emp;
--43.	Compute the total salary of all employees.

select sum(sal) as totalsalofallemp from emp;

--44.	Count the number of employees who have job titles(that is, the number of rows where JOB is not NULL).

select	count(ename) from emp where job is not null;

--45.	Compute the minimum and the maximum salary in the company.

select	top 1 e.ename, min(sal) as minsalary from EMP e group by ename order by minsalary;

select	top 1 ename,max(sal) as maxsalary from EMP e group by ename order by maxsalary desc;
...
select 
	ename as Employee, sal as Salary,job as Job,hiredate as 'Date of Hire',comm as Commission,
	deptno as 'Department No',mgr as 'MGR No'
from EMP;
...
select count(e.deptno) from emp e where e.deptno>10;

--49.	Write a query that will give you the names and jobs of all employees in New York with a commission above 10.

select ename,sal,comm from emp e join dept d on loc='NEW YORK' where e.comm > 10;
