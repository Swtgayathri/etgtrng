...
select first_name+' '+last_name as employess,department,worker_title from Worker_Details,title where worker_title='consultant';
...
inner join (select DEPARTMENT,max(salary) as maxsalary from Worker_Details
group by DEPARTMENT) MaxSalEmp on Worker_Details.department = MaxSalEmp.DEPARTMENT
